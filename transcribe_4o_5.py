...
# --- (D) 언어/파일명 ---
FILENAME_SUFFIX = "(3)"     # 최종 SRT 파일명 접미사
MAIN_LANGUAGE = "ko"
...
def define_third_language(segment_with_lang, target_languages):
    ...
    if lang_durations:
        ...
        if lang_durations[third_lang] < 30:
            third_lang = None
            print('제3언어 후보가 30초 미만 → 제3언어 없음')
...
def merge_unknown(segment_with_lang):
    ...
    merged = []
    for seg in segment_with_lang:
        if seg == segment_with_lang[0]:
            merged.append(seg)
        else:
            if seg['lang'] == 'unknown':
...
def split_text_max_chars(text: str, max_chars: int) -> List[str]:
    ...
    while len(s) > max_chars:
        ...
        m = re.finditer(r"[ \t]|[,.!?;:，。！？；：]", window)
        candidates = [x.start() for x in m]
        if candidates:
            cut = max(candidates)
            if cut < 1:
                cut = max_chars
...
def openai_diarize_segments(wav_path: str, language: Optional[str] = None) -> List[dict]:
    ...
    # 1) segments 뽑기 (SDK가 Pydantic 객체로 줄 수도 있음)
    ...
    for s in segs:
        ...
        # Pydantic(BaseModel)면 model_dump로 dict화
        if hasattr(s, "model_dump"):
            norm.append(s.model_dump())
            continue

        # 혹시 그 외 타입이면 attribute로 안전하게 뽑기
        norm.append({
...
def diarize_segments_to_cues(
    ...
    for seg in diar_segs:
        ...
        text = normalize_one_line(text)
        if include_speaker_prefix and speaker is not None:
            text = f"[{speaker}] {text}"
        ...
        for i, (c, ln) in enumerate(zip(chunks, lengths)):
            ...
            cues.append(Cue(start=cs + offset_sec, end=ce + offset_sec, text=c))
...
def extract_temp_wav_from_waveform(
    ...
    """
    waveform(1, T)에서 [start-pad, end+pad] 잘라 temp wav 저장.
    return: (temp_wav_path, chunk_offset_sec, chunk_end_sec)
    """
...
def run_openai_diarize_and_save_srt(
    ...
    sample_rate: int,
    audio_path: str,
    final_segments_3: List[dict],
    output_folder: str,
    done_path: str,
):
    """
    ✅ final_segment_3는 절대 건드리지 않고,
    각 세그먼트 오디오를 뽑아서 gpt-4o-transcribe-diarize로 diarize → 35자 1줄로 자막 구성.
    """
    print("\n🚀 OpenAI diarize STT 시작 (35자/1줄)...")
    ...
    try:
        for i, seg in enumerate(final_segments_3, 1):
            seg_start = float(seg["start"])
            seg_end = float(seg["end"])
            ...
            print(f"  - [{i}/{len(final_segments_3)}] {seg_start:.2f}s~{seg_end:.2f}s ({seg_lang}), dur={seg_dur:.2f}s")

            # 너무 긴 세그먼트는 내부 chunking (final_segment_3는 그대로 유지, STT만 나눠서 호출)
            cursor = seg_start
            chunk_idx = 0
            while cursor < seg_end - 0.01:
                ...
                # pad로 인해 범위 밖으로 튀는 cue를 원래 chunk_start~chunk_end로 클립
                clipped = []
                for c in cues:
...
if __name__ == "__main__":
    ...
    for i, audio_file in enumerate(list(audio_files), 1):
        ...
        final_segment_4 = final_merge_VAD_by_lang(
            final_segment_3,
            sub_language,
            third_lang,
            MAX_DURATION,
        ...
        # vad_annotation_to_srt_empty_with_lang(final_segment_4, output_path, audio_file, file_suffix="vad_view(2)")

        # ✅ 여기서부터 STT만 OpenAI diarize로 수행 (all_cues 루프 포함)
        run_openai_diarize_and_save_srt(
            waveform=waveform, sample_rate=sample_rate, audio_path=audio_file, 
            final_segments_3=final_segment_4, output_folder=output_path, done_path=done_path)
